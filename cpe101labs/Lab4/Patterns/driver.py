 ...
 *
"""
def compare_patterns(letter):
   pattern = read_pattern(sys.stdin)
   cols_list = [len(row) for row in pattern]
   computed_pattern = build_pattern(len(pattern), cols_list, letter)
   display_results(pattern, computed_pattern)
 ...
 *
"""
def validate_strings(strings, illegal):
   for row, string in enumerate(strings):
      match = illegal.search(string)
      if match:
         print("Unsupported character '{0}' in row {1}".format(match.group(),
            row), file=sys.stderr)
         sys.exit(1)
 ...
 * Special Notes: n/a
"""
def build_pattern(rows, cols_list, letter):
   pattern = []
 ...
 *
"""
def display_results(expected, computed):
